...
for index, row in lands[['zones', 'zoneRewards', 'winnerCount', 'citizenNftCount', 'levelUpPassCount', 'nftPerParticipant', 'lupPerParticipant']].iterrows():
    if (row['winnerCount']*row['lupPerParticipant'])>0:
        ...
        line_message += row['zones']+": "+str(round(reward*0.9,4))+" - "+str(round(reward*1.1,4))+"\n"
line_message += "\nEstimated rewards per NFT per 3LUP (after tax):\n"
for index, row in lands[['zones', 'zoneRewards', 'requiredLUP', 'winnerCount', 'citizenNftCount', 'nftPerParticipant']].iterrows():
    if row['requiredLUP']*3*0.7>0:
        line_message += row['zones']+": "+str(round(row['zoneRewards']*0.7/(row['winnerCount']*row['nftPerParticipant'])/row['requiredLUP']*3*0.7,2))+"\n"
print(line_notify(line_message))
